...
typename BinarySearchTree<TBA>::Node* BinarySearchTree<TBA>::insert(TBA in, Node* subtree) {
	...
	else if (in < subtree->data) subtree->left = insert(in, subtree->left);
	else subtree->right = insert(in, subtree->right);
	return subtree;
}
...
bool BinarySearchTree<TBA>::retrieve(TBA& toFind, TBA*& toStore, Node* subtree) {
	...
	else if (subtree->data > toFind) return retrieve(toFind, toStore, subtree->left);
	else return retrieve(toFind, toStore, subtree->right);
}
